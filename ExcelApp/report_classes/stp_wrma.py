...
def form_url(params):
	base_url = str(stp_config.CONST.API_URL_PREFIX) + 'bsmart_data/bsmart_data/stp_ws/stp_warranty_ma/'
	base_url += str(params["wtype"])
	return base_url

#Warranty Report Municipality Analysis
def render(res, params):
	...
	title = 'Warranty Report Municipality Analysis ' + type
		...
		for item_id, item in enumerate(data["items"]):
			if str(data["items"][item_id]["contractyear"]) == year:
				if data["items"][item_id]["municipality"] == mun and "warrantyaction" in data["items"][item_id]: 
					if data["items"][item_id]["species"] not in muns[mun]:
						muns[mun].update({data["items"][item_id]["species"] : [
								1,
						...
						muns[mun][data["items"][item_id]["species"]][0] += 1
						muns[mun][data["items"][item_id]["species"]][1] += 1 if data["items"][item_id].get("warrantyaction") == 'Accept' else 0
						muns[mun][data["items"][item_id]["species"]][2] += 1 if data["items"][item_id].get("warrantyaction") == 'Reject' else 0
	...
	for mid, mun in enumerate(muns):
		...
		worksheet.write_formula('B' + str(cr), '=SUM(B' + str(start) + ':B' + str(cr-1) + ')', format_text)
		worksheet.write_formula('C' + str(cr), '=SUM(C' + str(start) + ':C' + str(cr-1) + ')', format_text)
		worksheet.write_formula('D' + str(cr), '=SUM(D' + str(start) + ':D' + str(cr-1) + ')', format_text)
		worksheet.write_formula('E' + str(cr), '=SUM(E' + str(start) + ':E' + str(cr-1) + ')', format_text)
		cr += 2
